  ...
  return (
    <div id="live-camera-container" className="relative flex-1 h-full bg-zinc-950 flex items-center justify-center overflow-hidden">
      ...
      {(!isCameraActive || isLoading || cameraError) && (
        <div id="camera-status-overlay" className="absolute inset-0 bg-zinc-950/80 backdrop-blur-sm flex flex-col items-center justify-center p-6 text-center z-10">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center space-y-4 max-w-sm">
              ...
              <div>
                <h3 className="text-base font-semibold text-zinc-200">Start Hand Skeleton Tracking</h3>
                <p className="text-xs text-zinc-400 mt-1">Allow camera access to display the live feed with real-time skeleton overlay.</p>
              </div>
              <button
                id="btn-start-camera"
                onClick={onRetryCamera}
      ...
      {isCameraActive && (
        <div id="camera-active-badge" className="absolute top-4 left-4 z-10 flex items-center gap-2 px-3 py-1.5 rounded-full bg-zinc-900/80 backdrop-blur-md border border-zinc-800 text-zinc-300 text-xs font-medium">
          <span className="relative flex h-2 w-2">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
            <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
          </span>
          <span>Live Skeleton Feed</span>
        </div>
      )}
